...
| 批次     | 优先级      | 内容                                                                  | 状态  |
| ------ | -------- | ------------------------------------------------------------------- | --- |
| **A0** | 准备       | 确认 P0 范围; doc-only 关闭策略                                             | [ ] |
| **A1** | **P0**   | aidb **001+002** (WAL, 同测); aidb **005**; aikv **002** (生产 Options) | [ ] |
| **A2** | **P1**   | aikv 001, 006, 012, 013, 016 …                                      | [ ] |
...
```text
aidb 001+002 → aikv 002 → aikv 006 → 集群 013/016/014 → doc-only 清扫
```
...
| B1.2 | aidb: ISSUE-001/002 **WAL 崩溃恢复** 测试模板     | [ ]    |
| B1.3 | aikv: ISSUE-002 **生产 Options** 集成测        | [ ]    |
...
| 阶段          | 内容                                                     | 状态  |
| ----------- | ------------------------------------------------------ | --- |
| **B2-v0**   | `config.toml` 扫 aidb/aikv; 测试树; 单测运行+SSE; Docs+Mermaid | [ ] |
...
- [ ] **B1.1–B1.3** 测试基建 + **A1** P0 ISSUES (一体推进)
- [ ] **A0** 确认 ISSUES 批次
